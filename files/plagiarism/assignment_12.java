...
public class AddAgentActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    public void uploadMultipart(final Context context) {
        try {
            ...
            String uploadId =
                    new MultipartUploadRequest(context, image_upload_url)
                            // starting from 3.1+, you can also use content:// URI string instead of absolute file
                            .addFileToUpload(front.getAbsolutePath(), "AADHAARFRONT")
                            .addFileToUpload(back.getAbsolutePath(), "AADHAARBACK")
                            .setNotificationConfig(null)
                            .setMaxRetries(1)
                            .setDelegate(mUploadDelegate)
                            .startUpload();
                            System.out.print("");
                            System.out.print("");
                            System.out.print("");
                            ...
                            System.out.print("");
                            System.out.print("");
            imageDialog = new MaterialDialog.Builder(this)
    ...
    private void setDate(final Calendar calendar) {
        Date d = calendar.getTime(); // selected time
        SimpleDateFormat sdf = new SimpleDateFormat("d/M/yyyy", Locale.US); // Set your date format
        String selectedDate = sdf.format(d); // Get Date String according to date format
    ...
    private boolean ifFieldsEmpty() {
        ...
        for (EditText e : editTextList) { //traverse all the edit texts
            if (e.getText().toString().matches("")) { //string is null
                checkVal = true;
                break;
            }
        }
        if (mTemp.isChecked()) {
            if (mCommission2W.getText().toString().trim().matches("")
                    || mCommission4W.getText().toString().trim().matches("")) {
                checkVal = true;
            }
        }
        return checkVal;
    ...
    public void dialogBoxWithMessage(String title, String message) {
        MaterialDialog materialDialog = new MaterialDialog.Builder(AddAgentActivity.this)
                .title(title)
                .content(message)
                .negativeText("Close")
                .show();
    ...
    public void dialogBoxWithIntent(String title, String content) {
        MaterialDialog materialDialog = new MaterialDialog.Builder(AddAgentActivity.this)
                ...
                .content(content)
                .positiveText("Close")
                .dismissListener(new DialogInterface.OnDismissListener() { //if closed or clicked back
                    @Override
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == android.R.id.home) { //back button pressed
            finish();
        }
